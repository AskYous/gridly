...
from gridly.app import COLUMN_CAPS, MAX_WRAP_LINES, ROW_SIZES, GridlyApp
...
async def main():
    ...
    async with app.run_test(size=(120, 30)) as pilot:
        ...
        print("ellipsis, small rows :", heights(app), "| wrapping:", app._wrapping())
        assert heights(app) == [1, 1]

        # --- W alone grows the row; row_size is untouched
        await pilot.press("W"); await pilot.pause()
        print("wrap, row_size still :", app.row_size, "->", heights(app))
        assert app.row_size == "small", "wrapping should not need the row-size toggle"
        ...
        while app.column_width != "small":
            await pilot.press("w"); await pilot.pause()
        print("small cap            :", app.column_width, heights(app))
        ...
        while app.column_width != "unlimited":
            await pilot.press("w"); await pilot.pause()
        print("unlimited            :", app.column_width, heights(app), "| wrapping:", app._wrapping())
        assert not app._wrapping() and heights(app) == [1, 1]
        ...
        while app.column_width != "small":           # small cap -> a tall row
            await pilot.press("w"); await pilot.pause()
        ...
        assert heights(app) == [ROW_SIZES[app.row_size]] * 2
        await pilot.press("s"); await pilot.pause()
        print("s with ellipsis      :", app.row_size, heights(app))
